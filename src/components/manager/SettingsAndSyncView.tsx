...
export const SettingsAndSyncView: React.FC = () => {
  const {
    users,
    kpis,
    targets,
  ...
  const [activeSubTab, setActiveSubTab] = useState<'SHEETS' | 'AUDIT' | 'CONFIG'>('SHEETS');
  ...
  return (
    <div className="pb-24 pt-3 px-3 sm:px-6 max-w-5xl mx-auto space-y-4 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-3">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-slate-500 font-semibold">Google Sheets Database</span>
          </div>
          <h2 className="text-lg font-bold text-slate-900 mt-0.5">
            CƠ SỞ DỮ LIỆU & NHẬT KÝ HỆ THỐNG
          </h2>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between text-xs text-slate-600">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-200 font-bold text-xs flex items-center space-x-1 shrink-0 ml-2"
            >
      ...
      {/* SUB-TAB 2: AUDIT LOG (Section 27) */}
      {activeSubTab === 'AUDIT' && (
